...
static CO_ERR   COTNmtHbProdWrite(struct CO_OBJ_T *obj, struct CO_NODE_T *node, const void *buffer, uint32_t size);
...
static CO_ERR COTNmtHbProdWrite(struct CO_OBJ_T *obj, struct CO_NODE_T *node, const void *buffer, uint32_t size)
{
    ...
    ASSERT_EQU_ERR(size, COT_ENTRY_SIZE, CO_ERR_OBJ_SIZE);
